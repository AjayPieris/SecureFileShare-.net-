...
namespace SecureFileShare.Controllers
{
    public class FilesController : Controller
    {
        ...
        // sets up cloudinary connection using env variables
        ...
        // handles the file upload when form is submitted
        [HttpPost]
        public async Task<IActionResult> Upload(IFormFile file)
        {
            ...
            // generate a random id so filenames dont clash on cloudinary
            ...
            // save file info to mysql - storing publicId and url together separated by pipe
            var fileRecord = new FileRecord
            {
                OriginalName = file.FileName,
                SavedName = $"{publicId}|{uploadResult.SecureUrl}",
            ...
            ViewBag.Link = downloadLink;

            return View("Index");
        }

        // download endpoint - proxies the file so user cant see the real cloudinary url
        ...
        public async Task<IActionResult> Download(string link)
        {
            ...
            // handle old local files that were uploaded before cloudinary migration
            if (parts.Length == 1)
            {
            ...
            // for cloudinary files, fetch through server so direct url stays hidden
        ...
        // dashboard page - shows all uploaded files
        ...
        // deletes file from cloudinary and removes the db record
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (parts.Length == 1)
            {
                // old local file - just delete from disk
                string uploadsFolder = Path.Combine(_environment.WebRootPath, "uploads");
                string filePath = Path.Combine(uploadsFolder, fileRecord.SavedName);
                if (System.IO.File.Exists(filePath))
            ...
            {
                // cloudinary file - delete using the public id
